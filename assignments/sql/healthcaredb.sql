...
*/
 ...
 select companyName,planName,diseaseName,max(cnt)  "max_cnt"
 from(SELECT d.`diseaseName`"diseaseName",ic.`companyName`'companyName',ip.`planName`'planName',count(t.`claimID`) "cnt"
FROM disease d INNER JOIN treatment t ON d.`diseaseID`=t.`diseaseID` INNER JOIN claim c ON c.`claimID`=t.`claimID`
INNER JOIN insuranceplan ip  on c.uin = ip.uin INNER JOIN insurancecompany ic ON ic.`companyID`= ip.`companyID` 
GROUP BY d.`diseaseName`,ic.`companyName`,ip.`planName`) as v1 GROUP BY companyName,planName,diseaseName;

select companyName,planName,diseaseName,max(cnt) over(PARTITION BY companyName,planName) "min_nt"
 from(SELECT d.`diseaseName`"diseaseName",ic.`companyName`'companyName',ip.`planName`'planName',count(t.`claimID`) "cnt"
FROM disease d INNER JOIN treatment t ON d.`diseaseID`=t.`diseaseID` INNER JOIN claim c ON c.`claimID`=t.`claimID`
INNER JOIN insuranceplan ip  on c.uin = ip.uin INNER JOIN insurancecompany ic ON ic.`companyID`= ip.`companyID` 
GROUP BY d.`diseaseName`,ic.`companyName`,ip.`planName`) as v2;

DELIMITER $$
create PROCEDURE pro_1(cid INT)
BEGIN
DECLARE cname VARCHAR(50);
SELECT `companyName` INTO cname FROM insurancecompany WHERE `companyID` = cid;
SELECT companyName,planName,diseaseName,max(cnt) over(PARTITION BY companyName,planName) "max_cnt"
 FROM(SELECT d.`diseaseName`"diseaseName",ic.`companyName`'companyName',ip.`planName`'planName',count(t.`claimID`) "cnt"
FROM disease d INNER JOIN treatment t ON d.`diseaseID`=t.`diseaseID` INNER JOIN claim c ON c.`claimID`=t.`claimID`
INNER JOIN insuranceplan ip  on c.uin = ip.uin INNER JOIN insurancecompany ic ON ic.`companyID`= ip.`companyID` 
GROUP BY d.`diseaseName`,ic.`companyName`,ip.`planName`) as v1 WHERE `companyName` = cname;
END $$
DELIMITER ;
